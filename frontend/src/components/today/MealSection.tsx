...
export default function MealSection({ meal, items, onAdd, onDelete }: MealSectionProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setSearchErr(null);
      try {
        const res = await fetch(`/api/foods/search?q=${encodeURIComponent(search.trim())}`);
        if (res.ok) {
    ...
    <Card className="p-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 hover:text-emerald-700 py-1.5 px-3 rounded-lg hover:bg-emerald-50 transition-colors"
        >
          ...
          {items.map(i => (
            <li key={i.id} className="flex items-center justify-between text-sm py-1">
              <div className="min-w-0 flex-1">
                ...
                <span className="text-slate-400 text-xs"> · {i.qty}g</span>
              </div>
              <div className="flex items-center gap-3 shrink-0 ml-2">
                <span className="text-slate-500 tabular-nums text-xs">{i.calories} kcal</span>
                <button
      ...
      {open && (
        <div className="mt-3 pt-3 border-t border-slate-100 space-y-2">
          ...
          <div className="relative">
            {searching
              ? <Loader2 size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 animate-spin" />
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-2 rounded-lg border border-slate-200 text-sm text-slate-700 bg-slate-50 outline-none focus:border-emerald-400 transition-colors"
              autoFocus
            />
          </div>
          ...
          {/* Error */}
          {searchErr && (
            <p className="text-xs text-rose-500">{searchErr}</p>
          )}
          ...
          {results.length > 0 && (
            <div className="max-h-44 overflow-y-auto rounded-xl border border-slate-200 divide-y divide-slate-100">
              {results.map(f => (
                ...
                >
                  <span>{f.name}</span>
                  <span className="text-xs text-slate-400 ml-2 shrink-0 tabular-nums">
          ...
          {search.trim().length >= 2 && !searching && results.length === 0 && !searchErr && (
            <p className="text-xs text-slate-400 px-1">No results — try a different name</p>
          )}

          {/* Quantity + Add */}
          <div className="flex gap-2">
            <div className="flex-1 flex items-center rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
              <input
                type="number"
                value={qty}
            ...
            <button
              ...
              className="rounded-lg bg-emerald-600 px-5 py-2 text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-40 transition-colors"
            >
              Add
            </button>
          </div>
